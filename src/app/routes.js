...
function parseFilters(query) {
  const { periodo, from, to } = query;
  const where = {};
  if (from && to) where.fecha = { [Op.between]: [from, to] };
  else if (periodo) {
    const unit = { dia: 'day', semana: 'week', mes: 'month', anio: 'year' }[periodo] || 'month';
    where[Op.and] = sequelize.where(fn('date_trunc', unit, col('fecha')), fn('date_trunc', unit, fn('CURRENT_DATE')));
  }
  return { where };
}
